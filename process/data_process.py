...
from data_utils import load_raw_data, save_data

def parse_args():
    parser = argparse.ArgumentParser(description="Process raw EHR data.")

    # Task
    parser.add_argument("--raw", type=bool, default=True, help="Process raw data")
    parser.add_argument("--split", type=str, default=None, choices=["patient", "time"], help="Split data")

    # Input/Output
    parser.add_argument("--data_dir", type=str, default="/n/data1/hms/dbmi/manrai/aashna/SETPOINT/data/raw/mimiciv/3.1/", help="Directory to raw EHR data")
    parser.add_argument('--save_prefix', type=str, default='/n/data1/hms/dbmi/manrai/aashna/SETPOINT/data/processed/mimiciv/', help='Prefix of all saved files')
...
def process_raw_data(pt_df_merged, data_source):
    ...
    # top_pt_lab_freq = pt_lab_freq[pt_lab_freq["subject_id"] >= 0.01*len(df.subject_id.unique())]
...
def main():
    ...
    # Parse data source
    if "MIMIC" in args.data_dir.upper(): data_source = "MIMIC"
    elif "eICU" in args.data_dir.upper(): data_source = "eICU"
    else: raise NotImplementedError
