...
class NORMATrainer:
    ...
    def _extract_metrics(self, total_loss, total_forecast_loss, total_align_loss, total_r2, num_batches):
    ...
    def _process_batch(self, batch, is_training=True):
        ...
        elif isinstance(self.criterion, GaussianNLLLoss):
            # Simple Gaussian NLL loss
            loss_val = self.criterion(mu, log_var, y_true)
            forecast_loss = align_loss = loss_val
        elif isinstance(self.criterion, MSELoss):
            # Simple MSELoss
            loss_val = self.criterion(mu, y_true)
            forecast_loss = align_loss = loss_val
        else:
        ...
        return loss_val.item(), forecast_loss.item(), align_loss.item(), r2
    ...
    def _run_epoch(self, loader, is_training=True):
        ...
        total_loss = total_forecast_loss = total_align_loss = total_r2 = num_batches = 0
        
        pbar = tqdm(loader, desc=desc, leave=False)
        for batch in pbar:
            if is_training:
                loss_val, forecast_val, align_val, r2_val = self._process_batch(batch, is_training=True)
                pbar.set_postfix({
                    'Loss': f'{loss_val:.4f}',
                ...
                with torch.no_grad():
                    loss_val, forecast_val, align_val, r2_val = self._process_batch(batch, is_training=False)
        ...
        return self._extract_metrics(total_loss, total_forecast_loss, total_align_loss, total_r2, num_batches)
    ...
    def save_checkpoint(self, epoch, metrics, is_best=False):
        ...
        checkpoint_path = self.log_dir / f'checkpoint_latest.pth'
        torch.save(checkpoint, checkpoint_path)
        
        # Save best checkpoint
        if is_best:
            best_path = self.log_dir / f'checkpoint_best.pth'
