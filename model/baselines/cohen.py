...
event tables once with `python cohen.py --build_healthy`.
"""
import numpy as np
import pandas as pd
...
def _sex_idx(sex_val):
    """0 = male, 1 = female (matches get_sex_str convention in 05)."""
    if isinstance(sex_val, str):
        return 1 if sex_val[:1].upper() == "F" else 0
    return 1 if sex_val == 1 else 0
...
def _frac_within_popri(baseline, c):
    ...
    sex_key = baseline[c["sex"]].map(
        lambda s: ("F" if s[:1].upper() == "F" else "M") if isinstance(s, str)
        else ("F" if s == 1 else "M"))
...
def build_pair_table(split_df, min_bl=5):
    ...
    pairs["sex_idx"] = pairs["sex"].map(_sex_idx)
...
def load_dev_sequences(dev_dir, source="combined", nstates=2):
    ...
    for p in (scripts_dir, os.path.join(scripts_dir, "process"), model_dir):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location(
        "_model_data", os.path.join(model_dir, "data.py"))
    mod = importlib.util.module_from_spec(spec)
    # model/data.py does `from config import ...` expecting process/config;
    # drop any cached `config` (e.g. validation/config.py) for the exec
    ...
    train_seq, val_seq, test_seq = mod.load_and_split_data(
        dev_dir, source, print_info=True, nstates=nstates)
...
# ═══════════════════════════════════════════════════════════════════════════
# Their published exclusion lists, applied to the dev cohorts (MIMIC-IV,
# EHRSHOT) at the lab-point level; consumed by build_dev_frames_cohen_healthy
# above.  Was cohen_healthy.py until the 2026-09-08 cleanup.
#
#   ST2 (supp): 184 nonhealthy ICD-9/10 codes  -> a patient is nonhealthy from
#       6 months before their first such diagnosis onward ("not diagnosed with
#       any of the nonhealthy ICD-9 codes in the past or in the next 6 months")
#   ST3 (supp): 5,299 drug-lab pairs by med name -> a lab point is excluded if
#       the patient was exposed to a paired medication in the 6 months before
#       to 1 month after the test
...
#
# Build the event tables once with:  python cohen.py --build_healthy
