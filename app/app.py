...
# ---------------------------------------------------------------------------
# Multi-model support — lazy loading with cache
# ---------------------------------------------------------------------------
HF_REPO = 'aashnaps/NORMA'
DEVICE = None

# Cache: run_id -> {'model': model, 'hparams': hparams, 'is_quantile': bool}
_MODEL_CACHE = {}
_active_run_id = DEFAULT_RUN_ID

def _get_active_run_id():
    """Get run_id from request args, falling back to global default."""
    from flask import request as _req
    try:
        return _req.args.get('model_id', _active_run_id)
    except RuntimeError:
        return _active_run_id

def _ensure_model(run_id=None):
    """Load and cache a model by run_id. Returns (model, hparams, is_quantile)."""
    global DEVICE
    if run_id is None:
        run_id = _get_active_run_id()
    if run_id in _MODEL_CACHE:
        entry = _MODEL_CACHE[run_id]
        return entry['model'], entry['hparams'], entry['is_quantile']
    ...
    _MODEL_CACHE[run_id] = {
        'model': model, 'hparams': hparams, 'is_quantile': is_quantile,
    }
    print(f"Model {run_id} loaded. model={getattr(hparams, 'model', '?')}, "
          f"quantile={is_quantile}, normalize={getattr(hparams, 'normalize', False)}")
...
def _wi_point(test_name, sex01, age, history, t_last, horizon):
    ...
    mu, sigma, ci_lower, ci_upper = run_inference(test_name, sex01, age, history, t_next, 1)  # Normal state
    return {
        ...
        for seq in picks:
            try:
                ...
                for s_idx, s_label in STATE_LABELS.items():
                    mu, sigma, ci_lower, ci_upper = run_inference(test_name, sex01, age, history, t_next, s_idx)
                    preds[s_label] = {
                        'mu': round(mu, 3), 'sigma': round(sigma, 3),
                        'ci_lower': round(ci_lower, 3),
                        'ci_upper': round(ci_upper, 3),
...
def run_inference(test_name, sex01, age, history, t_next, s_next_int, run_id=None):
    """
    Run model inference for a single (test, patient, state) combination.

    history : list of {'day': float, 'value': float}, already sorted by day
    t_next  : float – days from first history measurement
    s_next_int : 0=low, 1=normal, 2=high
    run_id  : optional model run_id (defaults to active model)

    Returns dict with keys: mu, sigma, ci_lower, ci_upper
      - Gaussian models: CI from mu ± 1.96*sigma
      - Quantile models: CI from q2.5 and q97.5, mu=median, sigma=(q97.5-q2.5)/3.92
    """
    import torch
    if run_id is None:
        run_id = _get_active_run_id()
    model, hparams, is_quantile = _ensure_model(run_id)
    ...
    with torch.no_grad():
        output = model(x_h, s_h, t_h, sex_t, age_t, cid_t,
                       s_next_t, t_next_t, pad_mask=None)

    if is_quantile:
        # output: (1, 5) — [q2.5, q25, q50, q75, q97.5]
        q = output.squeeze(0).cpu().numpy()
        mu       = float(q[2])          # median
        ci_lower = float(q[0])          # q2.5
        ci_upper = float(q[4])          # q97.5
        sigma    = (ci_upper - ci_lower) / 3.92  # approximate sigma
    else:
        mu_t, lv_t = output
    ...
    return mu, sigma, ci_lower, ci_upper
...
def predict():
    ...
    run_id = data.get('model_id')  # optional: use a specific model run

    predictions = {}
    for s_idx, s_label in STATE_LABELS.items():
        try:
            mu, sigma, ci_lower, ci_upper = run_inference(test_name, sex01, age, history, t_next, s_idx, run_id=run_id)
            predictions[s_label] = {
                'mu':       round(mu, 3),
                'sigma':    round(sigma, 3),
                'ci_lower': round(ci_lower, 3),
    ...
    used_run_id = run_id if run_id else _get_active_run_id()
...
def sensitivity():
    ...
    for v in values:
        ...
        try:
            mu, sigma, ci_lower, ci_upper = run_inference(test_name, sweep_sex, sweep_age, history, sweep_t, 1, run_id=run_id)  # Normal state
            results.append({
                'value':    v,
                'mu':       round(mu, 3),
                'sigma':    round(sigma, 3),
...
def list_models():
    """List available model checkpoints (local only)."""
    models = []
    logs = Path(LOG_DIR)
    if logs.exists():
        for d in sorted(logs.iterdir()):
            best = d / 'checkpoint_best.pth'
            if best.exists():
                # Try to read metadata without loading torch
                meta_path = d / 'checkpoint_best.json'
                meta = {}
                if meta_path.exists():
                ...
                models.append({
                    ...
                    'val_loss': meta.get('metrics', {}).get('val', {}).get('loss', '?'),
                    'val_r2': meta.get('metrics', {}).get('val', {}).get('r2', '?'),
                    'train_data': hp.get('train', '?'),
                    'nstates': hp.get('nstates', hp.get('num_states', '?')),
                    'normalize': hp.get('normalize', False),
                    'active': d.name == _get_active_run_id(),
                })
    return jsonify({'models': models, 'active': _get_active_run_id()})

@app.route('/api/set_model/<run_id>')
def set_model(run_id):
    ...
    return jsonify({'active': _active_run_id})
